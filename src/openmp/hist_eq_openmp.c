...
int main(int argc, char **argv)
{
    ...
    if (!fp)
    {
        printf("Error: Could not open results/benchmarks/openmp_execution_results.txt for writing\n");
        return 1;
    }

    FILE *rmse_fp = fopen("results/benchmarks/openmp_rmse_results.txt", "w");
    if (!rmse_fp)
    {
        printf("Error: Could not open results/benchmarks/openmp_rmse_results.txt for writing\n");
        fclose(fp);
        return 1;
    }
    fprintf(rmse_fp, "# threads rmse\n");
    ...
    for (int i = 0; i < experiments; i++)
    {
        ...
        if (reference == NULL)
        {
            reference = read_pgm_p5("results/pgm/output_serial.pgm", &ref_w, &ref_h, &ref_maxval);
            if (!reference)
            {
                printf("Error: Could not read results/pgm/output_serial.pgm\n");
                free(img);
                fclose(rmse_fp);
                fclose(fp);
                return 1;
            ...
            if (ref_w != w || ref_h != h)
            {
                printf("Error: Serial and OpenMP images have different dimensions\n");
                free(reference);
                free(img);
                fclose(rmse_fp);
                fclose(fp);
        ...
        if (!write_pgm_p5(argv[2], img, w, h))
        {
            printf("Error: Could not write output image %s\n", argv[2]);
            free(img);
            free(reference);
            fclose(rmse_fp);
            fclose(fp);
        ...
        double rmse_value = compute_rmse(reference, img, w, h);
        fprintf(rmse_fp, "%d %f\n", threads, rmse_value);
        printf("Threads: %d Time: %f RMSE: %f\n", threads, time, rmse_value);
